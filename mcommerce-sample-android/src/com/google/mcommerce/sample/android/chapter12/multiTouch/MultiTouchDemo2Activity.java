...
public class MultiTouchDemo2Activity extends Activity implements OnTouchListener {
	private int os;
	...
	private void logAction(MotionEvent event) {
		...
		if (os > 7) {
			...
			if (ptrIndex_o != ptrIndex)
				Log.v(TAG, ">>> Pointer index mismatch: " + ptrIndex_o
						+ " (old) changed to " + ptrIndex);

			if (ptrId_o != ptrId)
				Log.v(TAG, ">>> Pointer Id mismatch: " + ptrId_o
						+ " (old) changed to " + ptrId);

			if (action_o != action)
				Log.v(TAG, ">>> True action value mismatch: " + action_o
						+ " (old) changed to " + action);
		}
	}
}